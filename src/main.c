...
int main(int argc, void *argv[]) {
  ...
  uint32_t trig_lev = 0x666; /* Corresponds to -250mV Threshold */
  ...
  int num_test_positions = 5; /* Loops num_acquisitions numerous times
                                 to test multiple parts of PMT face */
  int value; /* Will be used for functions which return ints rather than 
                CVErrorCodes */
  int cur_position;
...
CVIOSources Hit, CVIOSources Gate, CVIOSources Reset); Not Sure what GATE should be!*/ 
  ...
  ********************************************************************/
  ...
  for (cur_position = 0; cur_position < num_test_positions; cur_position++) {
    interrupts = 0;
    ...
    printf("Move analysis/*Ch_0 file to new folder. Press Enter when Done.");
    key = getchar();
  }
 /**************************************************  
